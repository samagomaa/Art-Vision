        ...
        if (file && file.type.startsWith('image/')) {
            ...
            try{
                const res = await axios.post('https://image-g3epahfrhjghgpfs.switzerlandnorth-01.azurewebsites.net/api/Prediction/predict-score', formData , {
                  ...
                  setdataAnalysis(res.data.data.predictions)
    ...
    :<> {image? <div className='container'>
        ...
        <div className="col-md-6 d-flex">
            ...
            <div className='border border-2 border-white rounded-2'>
                <p className='p-2 mb-0 text-white d-flex justify-content-between fw-bold'> <span>Aesthetic score :</span> {dataAnalysis.total < 0 ? 0 :dataAnalysis.total} /100 </p>
            </div>
            <div className='border border-2 border-white mt-3 rounded-2'>
                <p className='p-2 mb-0 text-white d-flex justify-content-between fw-bold'> <span>Color :</span> {dataAnalysis.color < 0 ? 0 : dataAnalysis.color} /10 </p>
            </div>
            <div className='border border-2 border-white mt-3 rounded-2'>
                <p className='p-2 mb-0 text-white d-flex justify-content-between fw-bold'> <span>Composition :</span> {dataAnalysis.composition < 0 ? 0 : dataAnalysis.composition} /10</p>
            </div>
            <div className='border border-2 border-white mt-3 rounded-2'>
                <p className='p-2 mb-0 text-white d-flex justify-content-between fw-bold'> <span>Texture :</span> {dataAnalysis.texture < 0 ? 0 : dataAnalysis.texture}/10 </p>
            </div>
